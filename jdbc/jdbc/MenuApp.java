    ...
    public static void connectionDuClient() {
        ...
        if (scanner.next().equals("y")) {
            ...
            try {
                ...
                String sql = "SELECT * FROM Client WHERE idclient = '" + id_client + "'";
                ResultSet rs = stmt.executeQuery(sql);
                if (rs.next()) {
                    MenuApp.id_client = rs.getString(0);
                    sql = "SELECT * FROM Clien_non_oublie WHERE idclient = '" + id_client + "'";
            ...
            clearScreen();
            System.out.println("Que faites vous ici, alors ?");
        }
    }
